...
namespace http {
    ...
    ScanResult Parser::parse_http_request( LineScanner& scanner, HTTPRequestData& request ) {
		...
		while (!request.finished()) {
			...
			size_t current_max_size = request.current_state() == RequestState::REQUEST_LINE
				...
				: MAX_HEADER_BLOCK_LEN - request.total_header_bytes();
			
			ScanResult r = scanner.scan(current_max_size);
			
			if (r != SUCCESS) return r;

			Base::Expected<RequestLine, int> rl_result(0);

			Base::Expected<std::pair<std::string, std::string>, int> header_result(0);
			...
			switch (request.current_state()) {
				...
				case RequestState::REQUEST_LINE:
					...
					rl_result = parse_request_line(scanner.line());
				...
				case RequestState::HEADERS:
					...
					header_result = parse_header(scanner.line());
